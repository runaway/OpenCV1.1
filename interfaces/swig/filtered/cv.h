...
/* 平面划分是将一个平面分割为一组互不重叠的能够复盖整个平面的区域P(facets)。上面
结构描述了建立在 2D 点集上的划分结构，其中点集互相连接并且构成平面图形，该图形通
过结合一些无限连接外部划分点(称为凸形点)的边缘，将一个平面用边按照其边缘划分成很
多小区域(facets)。 
对于每一个划分操作，都有一个对偶划分与之对应，对偶的意思是小区域和点(划分的顶点)
变换角色，即在对偶划分中，小区域被当做一个顶点(以下称之为虚拟点)，而原始的划分顶
点被当做小区域。在如下所示的图例中，原始的划分用实线来表示，而对偶划分用点线来表
示。 
OpenCV 使用Delaunay's 算法将平面分割成小的三角形区域。分割的实现通过从一个假定的
三角形(该三角形确保包括所有的分割点)开始不断迭代来完成。在这种情况下，对偶划分就
是输入的2d点集的 Voronoi图表。这种划分可以用于对一个平面的3d分段变换、形态变换、
平面点的快速定位以及建立特定的图结构 (比如 NNG,RNG等等)。 */
